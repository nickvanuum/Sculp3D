...
export default function CreateOrderForm() {
  ...
  // For non-custom presets: user-only additive hint (optional)
  ...
  // ✅ FIX: prevent confusing stale text when switching styles
  // This field is meant to be "extra optional hint", not the preset itself.
  // Clearing it on preset change makes the UI consistent and unambiguous.
  useEffect(() => {
    setExtraStyleHint("");
    setCustomStyleHint("");
  }, [preset]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // backend expects classical|modern|custom
      ...
      const finalStyleHint = (() => {
        if (isCustom) {
          return [
            ...
            // ✅ No pedestal injection + explicitly forbid separate pedestal/plinth objects
        ...
        // Note: extraStyleHint is optional and cleared on preset change
      ...
      if (usingPhone && phoneUploadToken) {
        fd.append("phoneUploadToken", phoneUploadToken);
      }
    ...
    <form onSubmit={submit} className="space-y-6">
          ...
          <input
            ...
            placeholder={
              isCustom
                ...
                : `Optional: e.g. "more dramatic curls", "cleaner hair", "stronger jawline"`
            }
            className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm focus-ring"
          />
          ...
          {isCustom ? (
            <div className="mt-2 space-y-2">
              <p className="text-xs text-slate-500">
                Custom works best when you include: material, era/style, what to remove/add (e.g.
                hoodie), and do/don’t instructions.
              </p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Choose exactly 1 sharp photo (front-lit, no filters).
          </p>
      ...
      <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-sm">
        ...
        <span className="text-slate-700">
          I confirm I have permission to use this photo to generate a 3D preview and produce a bust
          print.
        </span>
      </label>
